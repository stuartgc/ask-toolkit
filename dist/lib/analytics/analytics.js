 ...
 *
 */
const analyticsUtils = require("./analyticsUtils"), dashbotEventService = require("./dashbotEventService"), e = require("./../enums"), persistence = require("./../persistence/persistence"), utils = require("./../utils/utils");
const NOT_SENT = "Dashbot Event Not Sent", SESSION_END = "SessionEnd";
let provider = null;
const initProvider = () => {
...
const analytics = {
    ...
    "sendResponseTracking": {
        async process(handlerInput) {
            const event = utils.get("requestEnvelope", handlerInput, ""), requestAttr = handlerInput.attributesManager.getRequestAttributes();
            const notSent = utils.get("responseTrackingSent", requestAttr) !== true;
            const requestType = utils.get("request.type", event, ""), metadata = {
                slots: null,
    ...
    "sendEvent": async (event, trackingEvent, trackingParams) => {
    ...
    "sendSponsorEvent": async (event, campaignId, adLabel) => {
        return analyticsHandlers.sendEvent(event, "SponsorPlayedEvent", { campaignId: campaignId, label: adLabel });
    },
