...
const utils = {
    ...
    getResolvedValue: ( handlerInput, slotName ) => {
        const resolutionsPerAuthority = this.get( "requestEnvelope.request.intent.slots." + slotName + ".resolutions.resolutionsPerAuthority", handlerInput, null );

        if ( resolutionsPerAuthority[0] && resolutionsPerAuthority[0].values && resolutionsPerAuthority[0].values[0] ) {
            return this.get( "value.name", resolutionsPerAuthority[0].values[0], null )
        }
    ...
    getUserId: function( event, fallback ) {
        ...
        if ( event ) {
            return this.get( "session.user.userId", event, fallback );
        }
