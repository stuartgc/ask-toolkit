...
const listApi = {
    ...
    addItems: async ( handlerInput, listId, items ) => {
        ...
        if ( currentList ) {
                ...
                for ( let item of itemSet ) {
                    try {
                        ...
                        if ( error.statusCode === err.code.FORBIDDEN ) {
                            return listApiUtils.sendError( cb, error.statusCode, err.msg.NOT_AUTHORIZED );
                        } else {
                            return listApiUtils.sendError( cb, err.code.BAD_REQUEST, err.msg.METADATA_ERROR + " - " + error.message );
                        }
                    }
                }
            ...
            return listApiUtils.sendError( cb, err.code.UNAUTHORIZED, err.msg.NO_TOKEN );
        }
    },
