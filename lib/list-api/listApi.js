...
const Alexa = require( "alexa-sdk" ),
    analyticsHandlers = require( "./../analytics/analyticsHandlers" ),
    c = require( "./constants" ),
    err = require( "./errors" ),
    ...
    return {
        ...
        createItem: function( item, cb ) {
            ...
            if ( this.currentList[ item ] ) {
                ...
                .catch( ( error ) => {
                    utils.log.debug( "[ListApi::createItem] Error: " + JSON.stringify( error ) );
        ...
        fetchCurrentList: function( cb ) {
            ...
            if ( this.authToken && this.currentListId ) {
                lms.getList( this.currentListId, c.itemStatus.ACTIVE, this.authToken )
                .then( ( data ) => {
                ...
                .catch( ( error ) => {
                    utils.log.debug( "[ListApi::fetchCurrentList] Error: " + JSON.stringify( error ) );
            ...
            if ( this.authToken ) {
                ...
                .then( ( data ) => {
                    ...
                    if ( data.lists ) {
                        ...
                        delete this.scope.attributes[ c.attr.LIST_IDS ];
                    }

                    cb( true, data );
                } )
                .catch( ( error ) => {
                    utils.log.debug( "[ListApi::getListMetadata] Error: " + JSON.stringify( error ) );
         ...
         */
        hasListIds: function() {
            if ( this.scope.attributes[ c.attr.LIST_IDS ] ) {
                if ( utils.get( c.listTypes.TODO, this.scope.attributes[ c.attr.LIST_IDS ] ) &&  utils.get( c.listTypes.SHOPPING, this.scope.attributes[ c.attr.LIST_IDS ] ) ) {
                    return true;
                }
            }
        ...
        getListId: function( listType ) {
            let listId = utils.get( listType, this.scope.attributes[ c.attr.LIST_IDS ] );

            if ( !listId ) {
                listId = utils.get( c.listTypes.SHOPPING, this.scope.attributes[ c.attr.LIST_IDS ] );
            }
        ...
        setListIds: function( idList ) {
            this.scope.attributes[ c.attr.LIST_IDS ] = this.scope.attributes[ c.attr.LIST_IDS ] || {};

            for ( let i = 0; i < idList.length; i++ ) {
                if ( idList[ i ].name.indexOf( c.listTypes.TODO ) > -1 ) {
                    this.scope.attributes[ c.attr.LIST_IDS ][ c.listTypes.TODO ] = idList[ i ].listId;
                } else if ( idList[ i ].name.indexOf( c.listTypes.SHOPPING ) > -1 ) {
                    this.scope.attributes[ c.attr.LIST_IDS ][ c.listTypes.SHOPPING ] = idList[ i ].listId;
                }
            }
        },
...
const listApi = {
     ...
     *
     * @param listType...type of list (c.listTypes)
    ...
    sendPermissionCard: function( speechOutput, reprompt ) {
        if ( speechOutput ) {
            analyticsHandlers.sendEvent( this.event, c.events.SEND_PERMISSION_CARD, { type: c.events.TYPE, state: utils.get( "handler.state", this, "" ) }, () => {
                if ( reprompt ) {
                    this.emit( ":askWithLinkAccountCard", speechOutput, reprompt, c.PERMISSIONS );
                } else {
                    this.emit( ":tellWithPermissionCard", speechOutput, c.PERMISSIONS );
                }
            } );
        } else {
            this.emit( "::ask", {
