...
const makePlainText = new Alexa.PlainTextContentHelper(),
    makeImage = new Alexa.ImageHelper(),
    makeRichText = new Alexa.RichTextContentHelper();
...
const responseUtils = {
        ...
        if ( data && data.template ) {
            ...
            switch( data.template ) {
                case e.template.BODY_1:
                    utils.set( "textContent", template, this.buildTextObj( utils.get( "text", data ) ) );
                    break;
                case e.template.BODY_2:
                case e.template.BODY_3:
                case e.template.BODY_6:
                    utils.set( "image", template, this.buildImageObj( utils.get( "image", data ) ) );
                        ...
                        if ( items && Array.isArray( items ) ) {
                            for ( let i = 0; i < items.length; i++ ) {
                                ...
                                if ( utils.get( "image.url", items[ i ] ) ) {
                                    utils.set( "image", listItem, this.buildImageObj( utils.get( "image", items[ i ] ) ) );
                                }

                                utils.set( "textContent", listItem, this.buildTextObj( utils.get( "text", data ) ) );
            ...
            // Background Image
            utils.set( "backgroundImage", template, this.buildImageObj( responseUtils.getRandomItem( utils.get( "backgroundImage", data ) ) ) );
    ...
    buildImageObj: ( img ) => {
        if ( img ) {
            return makeImage
            .withDescription( utils.get( "description", img, null ) )
            .addImageInstance(
                utils.makeImagePath( utils.get( "url", img, null ) ),
                utils.get( "size", img, null ),
                utils.get( "width", img, null ),
                utils.get( "height", img, null )
            )
            .getImage();
        }
    ...
    buildTextObj: ( txt ) => {
        if ( txt ) {
            return makeRichText
            .withPrimaryText( utils.get( "primary", txt, null ) )
            .withSecondaryText( utils.get( "secondary", txt, null ) )
            .withTertiaryText( utils.get( "tertiary", txt, null ) )
            .getTextContent();
        }
